...
function ensureDigest(digest, label) {
  if (!SHA256_REGEX.test(digest))
    throw new Error(`Invalid digest for ${label}: ${digest}`);
}
...
function generatePublishMetadata(args) {
  ...
  if (!imageTag || !releaseVersion || !sourceSha || !outputPath)
    throw new Error('Missing required args for generate-publish-metadata');
...
function verifyTagParity(args) {
  ...
  if (!primaryTag || !secondaryTag)
    throw new Error('Missing required args for verify-tag-parity');
  ...
  for (const project of projects) {
    ...
    if (primaryDigest !== secondaryDigest) {
      throw new Error(
        `Digest mismatch for ${app} between ${primaryTag} and ${secondaryTag}`,
      );
    }
...
function generateReleaseManifest(args) {
  ...
  const outputPath = args['output'];
  if (!imageTag || !sourceSha || !artifactRunId || !outputPath)
    throw new Error('Missing required args for generate-release-manifest');
