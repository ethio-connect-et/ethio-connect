...
const BRANCH_POLICY = {
  ...
  scanFilePatterns: [/^\.github\/workflows\/.+\.ya?ml$/i, /^docs\/repository-rulesets\.md$/i],
  forbiddenPatterns: [
    {
      id: "legacy-branch-list-item",
      pattern: /^\s*-\s*(development|testing)\s*$/,
      message: "Legacy branch literal used as a raw list item outside canonical protected-branch definitions.",
    },
    {
    ...
    {
      id: "legacy-ref-eq",
      pattern: /github\.ref\s*==\s*['"]?refs\/heads\/(development|testing)['"]?/,
...
function shouldScan(filePath) {
  return BRANCH_POLICY.scanFilePatterns.some((pattern) => pattern.test(filePath));
}

function isAllowlisted(filePath, line) {
  return BRANCH_POLICY.allowContexts.some(({ file, line: linePattern }) => file.test(filePath) && linePattern.test(line));
}
...
function main() {
  ...
  if (violations.length > 0) {
    ...
    for (const violation of violations) {
      console.error(`- ${violation.filePath}:${violation.lineNumber} [${violation.ruleId}] ${violation.message}\n  ${violation.line.trim()}`);
    }
    process.exit(1);
  }

  console.log(`Branch policy checks passed for ${filesToScan.length} files.`);
  console.log(`Protected branches (canonical): ${BRANCH_POLICY.protectedBranches.join(", ")}`);
}
