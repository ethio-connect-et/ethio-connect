...
for (const app of config.applications) {
  ...
  const jsChunkFiles = fs.existsSync(chunksDir)
    ? fs.readdirSync(chunksDir).filter((file) => file.endsWith('.js')).map((file) => path.join(chunksDir, file))
    : [];
  const cssFiles = fs.existsSync(cssDir)
    ? fs.readdirSync(cssDir).filter((file) => file.endsWith('.css')).map((file) => path.join(cssDir, file))
    : [];
