  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      try {
        ...
        reject(new Error(`Failed to parse JSON from ${cmd} ${args.join(" ")}: ${err.message}`));
      }
    });
  });
}
...
async function validatePublishProjects(projects, reportPath) {
  ...
  for (const project of projects) {
    ...
    try {
      const projectConfig = await runJsonCommand("pnpm", ["nx", "show", "project", project, "--json"]);
      ...
      if (!target) {
        ...
        if (!entry.executor && !entry.command) {
          ...
          entry.message = "Target 'nx-release-publish' exists but has no executor or command.";
          hasFailures = true;
        } else {
...
function inferArchetype(project, relPath) {
  ...
  if (project.projectType === "application" && tags.has("release:docker")) return "deployableApp";
  return null;
}
...
async function validateArchetypes() {
  const files = (await findProjectJsonFiles(ROOT)).filter((f) => f.includes(`${path.sep}apps${path.sep}`) || f.includes(`${path.sep}libs${path.sep}`));
  ...
  for (const file of files) {
    ...
    for (const target of required) {
      ...
      if (!hasReason(requiredExemptions[target])) {
        violations.push(`${rel}: missing required target '${target}' for archetype '${archetype}' (add target or metadata.targetContract.requiredExemptions.${target})`);
      }
    }
  ...
  console.log(`Nx target contract validated for ${files.length} project.json files.`);
}
